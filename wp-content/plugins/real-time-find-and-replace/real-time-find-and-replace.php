...
function far_plugin_meta( $links, $file ) { // add some links to plugin meta row
	if ( strpos( $file, 'real-time-find-and-replace.php' ) !== false ) {
		...
		$links = array_merge( $links, array( '<a href="http://infolific.com/technology/software-worth-using/real-time-find-and-replace-for-wordpress/#pro-version" target="_blank">Pro Version (just $5)</a>' ) );
	}
	return $links;
}
	...
	<div id="far-sb">
		<div class="postbox" id="far-sbone">
			...
			<div class="inside">
				...
				<li>In the <a href="http://infolific.com/technology/software-worth-using/real-time-find-and-replace-for-wordpress/#pro-version" target="_blank">pro version</a> (just $5), you can further control when the replace occurs by specifying a querytring, referral, and post type to match. Note that these filters perform a simple "contains" for the text given.</li>
				</ol>
				<strong>Tips</strong>
				<ol>
		...
		<div class="postbox" id="far-sbthree">
			...
			<div class="inside">
				<ul>
					<li><a href="http://wordpress.org/extend/plugins/rss-includes-pages/">RSS Includes Pages</a>: Modifies RSS feeds so that they include pages and not just posts. My most popular plugin!</li>
					<li><a href="http://wordpress.org/extend/plugins/social-media-email-alerts/">Social Media E-Mail Alerts</a>: Receive e-mail alerts when your site gets traffic from social media sites of your choosing. You can also set up alerts for when certain parameters appear in URLs.</li>
				</ul>
			</div>
		</div>
	</div>
